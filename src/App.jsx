...
function Photo({ src, alt, className = '' }) {
  return <img className={`photo ${className}`} src={src} alt={alt} loading="lazy" decoding="async" />
}
  ...
  return (
    <main ref={root} className={entered ? 'site is-entered' : 'site'}>
      ...
      {!entered && (
        <section className="entry" aria-label="Experience introduction">
          ...
          <div className="entry-card">
            ...
            <button onClick={() => setEntered(true)} className="enter-button" aria-label="Open the Monsoon Letter">
      ...
      <header className="hero scene">
        <Photo src={p[0]} alt="Indian wedding couple portrait" className="hero-photo" />
        <div className="hero-wash" />
        <div className="hero-meta">
          <span>{story.couple.date}</span>
          <span>{story.couple.location}</span>
        </div>
        <div className="hero-title" aria-label={`${story.couple.first} and ${story.couple.second}`}>
